...
*/
component extends="wheels.migrator.Migration" hint="create link column" {

	function up() {
		transaction {
			try {
				addColumn(table = 'features', columnType = 'string', columnName = 'card_link', default = '', null = true);
			} catch (any e) {
				local.exception = e;
			}
	...
	function down() {
		transaction {
			try {
				removeColumn(table = 'features', columnName = 'card_link');
			} catch (any e) {
				local.exception = e;
			}
