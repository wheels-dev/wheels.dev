...
*/
component extends="wheels.migrator.Migration" hint="create cover image column" {

	function up() {
		transaction {
			try {
				addColumn(table = 'blog_posts', columnType = 'string', columnName = 'cover_image_path', default = '', null = true);
			} catch (any e) {
				local.exception = e;
			}
	...
	function down() {
		transaction {
			try {
				removeColumn(table = 'blog_posts', columnName = 'cover_image_path');
			} catch (any e) {
				local.exception = e;
			}
