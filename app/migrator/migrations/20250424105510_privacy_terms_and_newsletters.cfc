...
*/
component extends="wheels.migrator.Migration" hint="add_columns-for_privacy_terms_and_newsletters" {

	function up() {
		transaction {
			try {
				addColumn(table = 'users', columnType = 'boolean', columnName = 'agreed_privacy_policy', default = true, null = false);
				addColumn(table = 'users', columnType = 'boolean', columnName = 'newsletter_subscription', default = false, null = false);
			} catch (any e) {
				local.exception = e;
			}
	...
	function down() {
		transaction {
			try {
				removeColumn(table = 'users', columnName = 'agreed_privacy_policy');
				removeColumn(table = 'users', columnName = 'newsletter_subscription');
			} catch (any e) {
				local.exception = e;
			}
