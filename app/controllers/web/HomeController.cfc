...
component extends="app.Controllers.Controller" {
    ...
    function sitemap() {
        try {
            ...
            // Add blog posts
            var blogPosts = model("Blog").findAll(where="blog_posts.statusId <> #blogStatuses().DRAFT# AND blog_posts.status = 'Approved' AND blog_posts.publishedAt IS NOT NULL AND blog_posts.publishedAt <= '#now()#' ");
            ...
            var authors = model("Blog").findAll(
                ...
                include="User",
                distinct="true",
                where="blog_posts.statusId <> #blogStatuses().DRAFT# AND blog_posts.status = 'Approved' AND blog_posts.publishedAt IS NOT NULL AND blog_posts.publishedAt <= '#now()#' AND blog_posts.createdBy IS NOT NULL"
            );
